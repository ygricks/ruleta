...
export class Ruleta {
    ...
    roll() {
        ...
        const originalRect: Rect = this.zones.find(e=>(e.param.type===RectType.CELL && e.param.name===ball.toString()));
        originalRect
        ...
        if(Object.keys(this.bids).length) {
            for(const bid of Object.values(this.bids)) {
                ...
                if(multiplier) {
                    const winnerBid: IWinnerBid = {rect:bid.rect, amount:bid.amount, multiplier:multiplier, sum: bid.amount * multiplier};
